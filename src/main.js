...
function header() {
  return `
    ...
    <header class="site-header" id="site-header">
      ...
      <a class="button button--small desktop-cta" href="#inquire">Plan your stay</a>
    ...
    <div class="mobile-nav" id="mobile-nav" aria-hidden="true">
      ...
      <nav aria-label="Mobile navigation">
        <a href="/">Home</a>
        ${navItems.map(([label, href]) => `<a href="${href}">${label}</a>`).join('')}
        <a href="#inquire">Plan your stay</a>
      </nav>
...
function footer() {
  return `
    ...
    <a class="mobile-booking" href="#inquire"><span>Private estate in Indio</span><strong>Plan your stay</strong></a>`
}
  ...
  return `
    <section class="inquiry" id="inquire">
      <div class="inquiry__intro reveal">
        <p class="eyebrow eyebrow--light">Private inquiry</p>
        <h2>Tell us what<br />you’re imagining.</h2>
        <p>Share the dates, the people, and the reason for gathering. We’ll help shape the right kind of stay.</p>
      ...
      <form class="inquiry-form reveal" data-inquiry-form>
        ...
        <div class="field"><label for="arrival">Arrival</label><input id="arrival" name="arrival" type="date" /></div>
        <div class="field"><label for="departure">Departure</label><input id="departure" name="departure" type="date" /></div>
        <div class="field"><label for="guests">Guests</label><input id="guests" name="guests" type="number" min="1" inputmode="numeric" /></div>
        ...
        <div class="field field--wide"><label for="message">What should we know?</label><textarea id="message" name="message" rows="4" placeholder="Dates, priorities, questions, or anything that would make the stay exceptional."></textarea></div>
        <div class="field field--wide form-submit"><button class="button" type="submit">Prepare inquiry</button><p>For now, this securely prepares an email in your device’s mail app. A seamless on-page submission option is being finalized.</p></div>
      </form>
    </section>`
}
    ...
    <main id="main">
      <section class="home-hero">
        ...
        <div class="home-hero__content reveal is-visible">
          ...
          <div class="hero-actions"><a class="button" href="#inquire">Check your dates</a><a class="text-link text-link--light" href="#story">Discover the estate<span aria-hidden="true">↘</span></a></div>
        </div>
      ...
      <section class="story-grid section-pad">
        ...
        <div class="mosaic mosaic--estate reveal">
          <figure class="mosaic__wide">${picture(estateImages[5], 'Open-air kitchen and dining space at Sunset Club Ranch')}<figcaption>Open-air living</figcaption></figure>
          <figure>${picture(estateImages[1], 'Resort-style pool at Sunset Club Ranch')}<figcaption>Poolside days</figcaption></figure>
          <figure>${picture(estateImages[14], 'A private bedroom at Sunset Club Ranch')}<figcaption>Room to retreat</figcaption></figure>
          <figure class="mosaic__tall">${picture(estateImages[6], 'Outdoor kitchen and lounge at Sunset Club Ranch at dusk')}<figcaption>Desert afterglow</figcaption></figure>
        </div>
      </section>
      ...
      <section class="art-story">
        <div class="art-story__copy reveal">
          ...
          <p class="fine-print fine-print--light">Shown in current Sunset Club Ranch interiors.</p>
        </div>
      ...
      <section class="pathways section-pad">
        ...
        <div class="pathway-grid">
          ${[
            ['Stay together', 'A private compound for group escapes, family time, and long weekends.', '/stay.html', estateImages[5]],
            ['Celebrate here', 'Weddings, birthdays, and milestone gatherings begin with a private conversation.', '/weddings.html', estateImages[12]],
...
function stayPage() {
  return `
    <main id="main">
      ${pageHero({image: estateImages[3], eyebrow:'The private estate', title:'Three homes.<br /><em>One shared escape.</em>', copy:'A five-acre setting that gives the group a place to gather—and everyone enough space to find their own pace.', secondary:'See the homes'})}
      <section class="section-pad" id="story">
        ...
        <div class="home-cards">
          ${[
            ['01', 'The Main House', 'The social heart of the estate, centered on open living, shared meals, and direct access to the outdoors.', estateImages[7]],
            ['02', 'The Bunk House', 'An energetic home base with flexible sleeping arrangements and room for the crew to settle in.', estateImages[16]],
            ['03', 'The Barn House', 'A quieter counterpoint with generous bedrooms and a more tucked-away feeling.', estateImages[14]],
      ...
      <section class="new-estate section-pad">
        ...
        <p class="new-estate__note reveal">Fresh photography of these completed additions is being prepared. The site will show each one in full as the new image library arrives.</p>
      </section>
      <section class="section-pad">
        ${editorialHeader('A closer look', 'Details make the stay.')}
        <div class="gallery-grid">
          ${[estateImages[9], estateImages[11], estateImages[13], estateImages[14], estateImages[18], estateImages[19]].map((src, i) => `<button class="gallery-item reveal" type="button" data-lightbox="${src}" aria-label="Open estate photograph ${i + 1}">${picture(src, `Sunset Club Ranch detail ${i + 1}`)}</button>`).join('')}
        </div>
      </section>
      ${inquirySection('Private stay')}
    </main>`
...
function weddingsPage() {
  return `
    <main id="main">
      ${pageHero({image: estateImages[12], eyebrow:'Weddings & celebrations', title:'Gather beautifully.<br /><em>Stay completely.</em>', copy:'An intimate desert setting for weddings, birthdays, and milestone weekends—with the estate reserved around your people.', cta:'Inquire about your date', secondary:'Imagine the weekend'})}
      <section class="intro split" id="story"><div class="intro__copy reveal"><p class="eyebrow">The occasion</p><h2>Not a ballroom.<br />A place that feels like yours.</h2><p class="lead">Celebrate under open sky, gather around the table, and let the weekend unfold without separating everyone at the end of the night.</p><p class="fine-print">All event use is reviewed individually and remains subject to applicable permits, insurance, parking, noise, and property requirements.</p></div><figure class="editorial-image editorial-image--portrait reveal">${picture(estateImages[15], 'Outdoor gathering space at Sunset Club Ranch')}<figcaption>Five acres for a more personal kind of gathering</figcaption></figure></section>
...
function corporatePage() {
  return `
    <main id="main">
      ${pageHero({image: estateImages[4], eyebrow:'Corporate retreats', title:'Better ideas need<br /><em>better room.</em>', copy:'Trade the ballroom for five private acres, three homes, open-air conversations, and a setting built for teams to reconnect.', cta:'Plan a retreat', secondary:'See the format'})}
      ...
      <section class="work-play split split--dark"><figure class="editorial-image reveal">${picture(estateImages[5], 'Outdoor kitchen and meeting space at Sunset Club Ranch')}</figure><div class="intro__copy reveal"><p class="eyebrow eyebrow--light">Beyond the agenda</p><h2>Work that leaves room for the people doing it.</h2><p class="lead">Morning conversation. A long table. Time outside. A retreat should create momentum without recreating the office.</p></div></section>
...
function afterDarkPage() {
  return `
    <main id="main" class="night-page">
      ...
      <section class="full-bleed-statement full-bleed-statement--night">${picture(estateImages[6], 'Outdoor kitchen and lounge at Sunset Club Ranch after sunset')}<div><p class="eyebrow eyebrow--light">Week one · 2027</p><h2>A very different<br />festival basecamp.</h2></div></section>
  ...
  return `
    <main id="main">
      ...
      <section class="underway-section">
        <div class="underway-section__copy reveal"><p class="concept-badge">Improvements underway</p><p class="eyebrow eyebrow--light">The landscape after sunset</p><h2>A brighter arrival.<br />A more cinematic night.</h2><p>New tree lighting, an illuminated ficus-lined driveway, and added waterfall features at the pool are now being developed across the estate.</p><p class="fine-print fine-print--light">Images shown are design visualizations. Final appearance and completion timing may vary; these features will be marked available only after installation is complete.</p></div>
        <div class="underway-section__media reveal"><figure>${picture('/media/vision-2027/arrival-night-vision.webp','Concept visualization of the planned illuminated ficus-lined arrival at Sunset Club Ranch')}<figcaption>Arrival lighting concept</figcaption></figure><figure>${picture('/media/vision-2027/estate-evolution-aerial.webp','Concept visualization of outdoor improvements at Sunset Club Ranch')}<figcaption>Outdoor evolution concept</figcaption></figure></div>
      </section>
